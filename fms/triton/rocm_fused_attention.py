...
def _attn_fwd_inner(
    ...
    for start_n in range (block_min, block_max, BLOCK_N):
        ...
        k = load_fn(K_block_ptr, PADDED_HEAD, MASK_STEPS and (n_extra_tokens != 0), DEFAULT_PADDING_OPTION)
        if PRE_LOAD_V:
            v = load_fn(V_block_ptr, MASK_STEPS and (n_extra_tokens != 0), PADDED_HEAD, DEFAULT_PADDING_OPTION)
        ...
        if bias_ptr is not None:
            bias = load_fn(bias_ptr, False, MASK_STEPS and (n_extra_tokens != 0), DEFAULT_PADDING_OPTION)
   ...
   configs=[
    ...
    #    triton.Config({'BLOCK_M': 128, 'BLOCK_N': 128, 'PRE_LOAD_V': False}, num_stages=1, num_warps=4),
       ...
       #triton.Config({'BLOCK_M': 16,  'BLOCK_N': 16,  'PRE_LOAD_V': False}, num_stages=1, num_warps=4),
        triton.Config({'BLOCK_M': 128, 'BLOCK_N':  64, 'PRE_LOAD_V':  True}, num_warps=2, num_stages=4),
        triton.Config({'BLOCK_M': 256, 'BLOCK_N':  64, 'PRE_LOAD_V': False}, num_warps=8, num_stages=8),
        triton.Config({'BLOCK_M': 128, 'BLOCK_N': 128, 'PRE_LOAD_V': False}, num_warps=4, num_stages=8),
        triton.Config({'BLOCK_M': 128, 'BLOCK_N':  64, 'PRE_LOAD_V':  True}, num_warps=4, num_stages=1),
        # triton.Config({'BLOCK_M':  16, 'BLOCK_N':  16, 'PRE_LOAD_V': False}, num_warps=2, num_stages=1),
        triton.Config({'BLOCK_M': 128, 'BLOCK_N':  64, 'PRE_LOAD_V': False}, num_warps=4, num_stages=2),
        triton.Config({'BLOCK_M': 128, 'BLOCK_N':  64, 'PRE_LOAD_V':  True}, num_warps=4, num_stages=8),
        triton.Config({'BLOCK_M':  32, 'BLOCK_N':  32, 'PRE_LOAD_V': False}, num_warps=8, num_stages=4),
        triton.Config({'BLOCK_M': 128, 'BLOCK_N':  64, 'PRE_LOAD_V': False}, num_warps=4, num_stages=4),
        triton.Config({'BLOCK_M': 128, 'BLOCK_N':  64, 'PRE_LOAD_V': False}, num_warps=2, num_stages=1),
        triton.Config({'BLOCK_M': 128, 'BLOCK_N':  64, 'PRE_LOAD_V':  True}, num_warps=2, num_stages=2),
        triton.Config({'BLOCK_M': 128, 'BLOCK_N':  64, 'PRE_LOAD_V':  True}, num_warps=4, num_stages=4),
        triton.Config({'BLOCK_M': 128, 'BLOCK_N': 128, 'PRE_LOAD_V': False}, num_warps=4, num_stages=2),
        triton.Config({'BLOCK_M': 256, 'BLOCK_N': 128, 'PRE_LOAD_V': False}, num_warps=8, num_stages=4),
        triton.Config({'BLOCK_M': 128, 'BLOCK_N':  64, 'PRE_LOAD_V':  True}, num_warps=4, num_stages=1),
        # triton.Config({'BLOCK_M':  16, 'BLOCK_N':  16, 'PRE_LOAD_V': False}, num_warps=4, num_stages=4),
        triton.Config({'BLOCK_M':  32, 'BLOCK_N':  32, 'PRE_LOAD_V': False}, num_warps=8, num_stages=2),
        triton.Config({'BLOCK_M': 256, 'BLOCK_N': 128, 'PRE_LOAD_V': False}, num_warps=8, num_stages=2),
    ],
#    key=['hq', 'hk', 'IS_CAUSAL', 'dropout_p', 'BLOCK_DMODEL'],
    key=['hq', 'hk', 'IS_CAUSAL', 'dropout_p', 'BLOCK_DMODEL', 
        ...
        'MAX_SEQLENS_Q', 'MAX_SEQLENS_K', 'VARLEN'
        ],
   use_cuda_graph=True,
)
@triton.jit
def attn_fwd(
    ...
    q = load_fn(Q_block_ptr, True, padded_head, DEFAULT_PADDING_OPTION)
